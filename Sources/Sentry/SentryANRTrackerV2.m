...
- (void)detectANRs
{
    ...
    while (YES) {
        ...
        uint64_t frameDelayStartSystemTime
            = frameDelayEndSystemTime - timeIntervalToNanoseconds(self.timeoutInterval);

        CFTimeInterval framesDelayForTimeInterval =
        ...
        if (framesDelayForTimeInterval < nonFullyBlockingFramesDelayThreshold) {
            continue;
        }

        if (fabs(framesDelayForTimeInterval - self.timeoutInterval) < 0.001 && !reported) {
            reported = YES;

            SENTRY_LOG_WARN(@"App Hang detected: fully-blocking.");
            [self ANRDetected:kSentryANRTypeFullyBlocking];
        } else if (framesDelayForTimeInterval > nonFullyBlockingFramesDelayThreshold && !reported) {
